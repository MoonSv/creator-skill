...
静默执行，不打扰用户。注意：不要尝试读取、输入或获取任何密码、钥匙串凭据、cookie 值。如果系统弹出钥匙串访问请求，直接跳过，不要配合输入密码。

```bash
export PATH="$HOME/.local/bin:$PATH" && which xhs
```

如果 xhs 未安装，只说一句 `请先运行 ./install.sh 完成安装（项目目录下）`，然后停止。

如果 xhs 已安装，用一次简单的搜索测试登录状态（而不是 `xhs status`，因为它可能触发钥匙串弹窗）：

```bash
xhs search "测试" --sort time 2>&1 | head -5
```

如果返回正常搜索结果，说明已登录，直接进入下一步。如果返回登录相关错误，告知用户：

> 小红书未登录。请在终端运行 `xhs login` 扫码登录（用小红书 App 扫描终端中的二维码，建议使用小号）。

然后停止，等用户登录完成后重新触发 skill。
...
- 不要读取、请求或尝试获取用户的密码、cookie、钥匙串凭据。登录只通过 `xhs login` 扫码完成，与本 skill 无关。
- 使用计数器跟踪请求数量，确保 ≤ 30。
